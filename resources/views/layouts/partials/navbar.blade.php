    <nav class="navbar navbar-default">
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                {{--arama cubugunda arama yapılcak yer--}}
                <form class="navbar-form navbar-left" action="{{route('urun_ara')}}" method="post">
                    {{--form güvenliğini saglamak için belirli bir alan kullanmaktadır.bu csrf_field() alanı eklemezsek laravel post işlemini durdurmaktadır. --}}
                    {{ csrf_field() }}
                ...
                <ul class="nav navbar-nav navbar-right">
                    <li><a href="{{ route('sepet') }}">
                        <i class="fa fa-shopping-cart"></i>Sepet 
                        {{-- count fonknu sepetteki toplam adedi gösterir --}}
                        <span class="badge badge-theme">{{ Cart::count() }}</span> 
                    </a></li>
                    {{--helper tanımları guest ve auth--}}
                    {{-- guest tanımı  ->oturum açmayan kullanıcıların görmesini istediğimiz yerleri belirtir--}}
                    @guest
                    <li><a href="{{ route('kullanici.oturumac') }}">Oturum Aç</a></li>
                    <li><a href="{{ route('kullanici.kaydol') }}">Kaydol</a></li>
                    @endguest

                    {{-- auth ise oturum açmış kullanıcıların görmesini istediğimiz kısımlardan oluşur --}}
                    @auth
                    <li class="dropdown">
